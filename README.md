...
### 🕵️ 誰是臥底 `/undercover` — 74 組內建詞語對 ＋ 領袖自訂
...
1. 領袖每局按「抽下一局代碼」時，才用 `crypto.getRandomValues`（密碼學亂數）即場抽出一個 **6 位本局代碼**，按掣前連領袖自己都無法預知
2. 領袖讀出／展示代碼，玩家在手機鍵盤輸入，即見本局身分
3. 所有裝置以 `hash(牌局密鑰 ‖ 本局代碼)` 推導出同一組結果，故毋須伺服器亦保證各機一致
4. 「牌局密鑰」只存在於各人自己的 QR 內，單靠聽到代碼並不能推算任何人的身分

> **結論**：局與局之間完全獨立，**沒有任何可以背誦的派發順序**。
> 已用 10,000 局模擬驗證：各座位當臥底的次數分布均勻（卡方值 3.39，遠低於 5% 臨界值 11.07），
> 連續兩局同一人做臥底的機率 16.4%，與理論值 1/6 相符。
