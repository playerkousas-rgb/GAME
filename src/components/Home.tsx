/**
 * 主頁 — 遊戲中心（Hub）
 * Copyright (c) 2026 Scout System. All rights reserved.
 */
import { Link } from 'react-router-dom'
import { Eye, Image as ImageIcon, ArrowRight, Users, Clock, Monitor } from 'lucide-react'
import { BRAND } from '../shared/brand'
import Footer from './Footer'

type GameCard = {
  to: string
  icon: React.ReactNode
  emoji: string
  title: string
  subtitle: string
  desc: string
  tags: string[]
  accent: string
  glow: string
}

const GAMES: GameCard[] = [
  {
    to: '/kims',
    icon: <Eye className="h-5 w-5" />,
    emoji: '👁️',
    title: '童軍金氏遊戲',
    subtitle: "Scout Kim's Games",
    desc: '視覺、聽覺、文字、配對四合一的觀察與記憶訓練，內建物品庫、小隊積分榜與 Kahoot 風格比賽模式。',
    tags: ['觀察力', '記憶力', '小隊比拼'],
    accent: 'from-amber-500/20 to-amber-700/5 border-amber-500/30 hover:border-amber-400/60',
    glow: 'group-hover:shadow-amber-500/20',
  },
  {
    to: '/photo',
    icon: <ImageIcon className="h-5 w-5" />,
    emoji: '🖼️',
    title: '像素化猜謎圖',
    subtitle: 'Photo Guessing Game',
    desc: '上傳相片後以像素化、局部放大、變形扭曲、切割打亂四種方式出題，逐級降低難度讓成員搶答。',
    tags: ['搶答', '投影專用', '自訂圖片'],
    accent: 'from-indigo-500/20 to-indigo-700/5 border-indigo-500/30 hover:border-indigo-400/60',
    glow: 'group-hover:shadow-indigo-500/20',
  },
]

const FEATURES = [
  { icon: <Monitor className="h-4 w-4" />, label: '投影友好', desc: '全屏模式，適合大螢幕' },
  { icon: <Users className="h-4 w-4" />, label: '小隊計分', desc: '個人 / 小隊 / 比賽模式' },
  { icon: <Clock className="h-4 w-4" />, label: '彈性限時', desc: '自由設定觀察與作答時間' },
]

export default function Home() {
  return (
    <div className="flex min-h-screen flex-col bg-[#02133e] text-white">
      {/* HERO */}
      <div className="relative overflow-hidden border-b border-white/10">
        <div className="pointer-events-none absolute -left-24 -top-24 h-72 w-72 rounded-full bg-amber-500/10 blur-3xl" />
        <div className="pointer-events-none absolute -right-24 top-0 h-72 w-72 rounded-full bg-indigo-500/10 blur-3xl" />
        <div className="relative mx-auto max-w-6xl px-4 py-14 text-center">
          <div className="mx-auto mb-4 flex h-14 w-14 items-center justify-center rounded-2xl bg-amber-400 text-3xl shadow-lg shadow-amber-500/20">
            ⚜
          </div>
          <h1 className="text-3xl font-black tracking-tight md:text-5xl">{BRAND.name}</h1>
          <p className="mt-2 text-base text-white/60 md:text-lg">
            {BRAND.nameZh} · {BRAND.tagline}
          </p>
          <div className="mt-5 flex flex-wrap items-center justify-center gap-2">
            {FEATURES.map((f) => (
              <div
                key={f.label}
                className="flex items-center gap-1.5 rounded-full border border-white/10 bg-white/5 px-3 py-1.5 text-xs text-white/60"
              >
                <span className="text-amber-400">{f.icon}</span>
                {f.label}
              </div>
            ))}
          </div>
        </div>
      </div>

      {/* GAME CARDS */}
      <main className="mx-auto w-full max-w-6xl flex-1 px-4 py-10">
        <h2 className="mb-4 flex items-center gap-2 text-sm font-semibold text-white/80">
          <span className="text-amber-400">🎮</span> 選擇遊戲
        </h2>
        <div className="grid gap-4 md:grid-cols-2">
          {GAMES.map((g) => (
            <Link
              key={g.to}
              to={g.to}
              className={`group relative overflow-hidden rounded-2xl border bg-gradient-to-br p-6 shadow-lg shadow-black/20 transition-all hover:scale-[1.015] ${g.accent} ${g.glow}`}
            >
              <div className="mb-3 flex items-start justify-between">
                <div className="text-4xl">{g.emoji}</div>
                <ArrowRight className="h-5 w-5 text-white/25 transition group-hover:translate-x-1 group-hover:text-white/70" />
              </div>
              <h3 className="text-xl font-black">{g.title}</h3>
              <p className="mt-0.5 text-[11px] uppercase tracking-wider text-white/35">{g.subtitle}</p>
              <p className="mt-3 text-sm leading-relaxed text-white/60">{g.desc}</p>
              <div className="mt-4 flex flex-wrap gap-1.5">
                {g.tags.map((t) => (
                  <span
                    key={t}
                    className="rounded-full border border-white/10 bg-black/20 px-2 py-0.5 text-[10px] text-white/50"
                  >
                    {t}
                  </span>
                ))}
              </div>
            </Link>
          ))}
        </div>

        {/* INFO */}
        <div className="mt-8 grid gap-3 md:grid-cols-3">
          {FEATURES.map((f) => (
            <div key={f.label} className="rounded-xl border border-white/10 bg-white/5 p-4">
              <div className="mb-1.5 flex items-center gap-2 text-sm font-semibold">
                <span className="text-amber-400">{f.icon}</span>
                {f.label}
              </div>
              <p className="text-xs text-white/45">{f.desc}</p>
            </div>
          ))}
        </div>

        <div className="mt-8 rounded-xl border border-white/10 bg-white/[0.03] p-5">
          <h3 className="mb-2 text-sm font-semibold">📋 更多集會小遊戲建議</h3>
          <p className="text-xs leading-relaxed text-white/50">
            倉庫內附有 <code className="rounded bg-black/30 px-1.5 py-0.5 text-amber-300">docs/集會小遊戲建議.md</code>
            ，收錄 20+ 個適合旅團集會的破冰、團隊合作、技能訓練及室內外遊戲，含人數、時間、器材與玩法說明。
          </p>
        </div>
      </main>

      <Footer />
    </div>
  )
}
