    ...
    <div className="rounded-xl border border-blue-700/30 bg-[#0a1e4a]/60 p-3">
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs text-blue-400">共 {items.length} 件</span>
          <span className="text-xs text-blue-500">(內建 {builtInActive.length} + 自訂 {customItems.length})</span>
        </div>
      ...
      {showAddForm && (
        <div className="mb-3 rounded-lg border border-amber-500/30 bg-amber-900/20 p-3 space-y-2">
          <div className="grid grid-cols-2 gap-2">
            <div>
              <label className="text-[10px] text-blue-400">名稱</label>
          ...
          <div className="grid grid-cols-2 gap-2">
            <div>
              <label className="text-[10px] text-blue-400">分類</label>
            ...
            <div>
              <label className="text-[10px] text-blue-400">難度</label>
          ...
          <div className="flex gap-2">
            ...
            <button onClick={() => setShowAddForm(false)} className="px-3 rounded border border-blue-700/50 py-1.5 text-xs text-blue-300">取消</button>
          </div>
        </div>
      )}
      ...
      {/* 上傳圖片按鈕 */}
      <div className="mb-2">
        <label className="flex items-center gap-1.5 rounded-lg border border-dashed border-blue-700/40 px-3 py-1.5 text-xs text-blue-300 cursor-pointer hover:border-amber-400/50 transition-colors">
          <Upload size={12} />
          上傳圖片轉為物品
      ...
      <div className="max-h-48 overflow-y-auto space-y-0.5">
        {filteredItems.length === 0 && (
          <p className="text-xs text-blue-500 text-center py-4">沒有符合的物品</p>
        )}
        {filteredItems.map(item => (
          <div key={item.id} className="flex items-center justify-between rounded-lg px-2 py-1 hover:bg-blue-900/30 group text-xs">
            <div className="flex items-center gap-2 min-w-0">
              ...
              <span className="text-blue-500 hidden md:inline text-[10px]">({item.category})</span>
              {item.isCustom && <span className="text-amber-500 text-[10px]">自訂</span>}
            </div>
            <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
              {item.builtIn && (
                <span className="text-[10px] text-blue-600">內建</span>
              )}
              <button
