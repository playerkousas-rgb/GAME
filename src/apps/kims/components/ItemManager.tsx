...
export default function ItemManager({ items, onItemsChange, onUploadImage }: Props) {
  ...
  return (
    <div className="rounded-xl border border-blue-700/30 bg-[#0a1e4a]/60 p-3">
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          <span className="text-sm font-medium text-blue-200">📦 物品庫管理</span>
          <span className="text-xs text-blue-200">共 {items.length} 件</span>
          <span className="text-xs text-blue-200">(內建 {builtInActive.length} + 自訂 {customItems.length})</span>
        </div>
      ...
      <div className="relative mb-2">
        ...
        <input
          ...
          placeholder="搜尋物品..."
          className="w-full rounded-lg border border-blue-700/50 bg-[#02133E] py-1.5 pl-7 pr-3 text-xs text-white placeholder-blue-500 focus:border-amber-400/50 focus:outline-none"
        />
      </div>

      {/* 新增表單 */}
      {showAddForm && (
        <div className="mb-3 rounded-lg border border-amber-500/30 bg-amber-900/20 p-3 space-y-2">
          <div className="grid grid-cols-2 gap-2">
            <div>
              <label className="text-[10px] text-blue-200">名稱</label>
              <input value={newName} onChange={e => setNewName(e.target.value)} placeholder="物品名稱" className="w-full rounded border border-blue-700/50 bg-[#02133E] p-1.5 text-xs text-white" />
            </div>
            <div>
              <label className="text-[10px] text-blue-200">Emoji</label>
              <div className="flex gap-1">
          ...
          <div className="grid grid-cols-2 gap-2">
            <div>
              <label className="text-[10px] text-blue-200">分類</label>
              <select value={newCategory} onChange={e => setNewCategory(e.target.value)} className="w-full rounded border border-blue-700/50 bg-[#02133E] p-1.5 text-xs text-white">
                {CATEGORIES.map(c => <option key={c} value={c}>{c}</option>)}
              </select>
            </div>
            <div>
              <label className="text-[10px] text-blue-200">難度</label>
              <select value={newLevel} onChange={e => setNewLevel(e.target.value as Difficulty)} className="w-full rounded border border-blue-700/50 bg-[#02133E] p-1.5 text-xs text-white">
          ...
          <div className="flex gap-2">
            ...
            <button onClick={() => setShowAddForm(false)} className="px-3 rounded border border-blue-700/50 py-1.5 text-xs text-blue-100">取消</button>
          </div>
        </div>
      )}
      ...
      {/* 上傳圖片按鈕 */}
      <div className="mb-2">
        <label className="flex items-center gap-1.5 rounded-lg border border-dashed border-blue-700/40 px-3 py-1.5 text-xs text-blue-100 cursor-pointer hover:border-amber-400/50 transition-colors">
          <Upload size={12} />
          上傳圖片轉為物品
      ...
      <div className="max-h-48 overflow-y-auto space-y-0.5">
        {filteredItems.length === 0 && (
          <p className="text-xs text-blue-200 text-center py-4">沒有符合的物品</p>
        )}
        {filteredItems.map(item => (
          <div key={item.id} className="flex items-center justify-between rounded-lg px-2 py-1 hover:bg-blue-900/30 group text-xs">
            <div className="flex items-center gap-2 min-w-0">
              ...
              <span className="text-blue-200 hidden md:inline text-[10px]">({item.category})</span>
              {item.isCustom && <span className="text-amber-500 text-[10px]">自訂</span>}
            </div>
            <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
              {item.builtIn && (
                <span className="text-[10px] text-blue-100">內建</span>
              )}
              <button
